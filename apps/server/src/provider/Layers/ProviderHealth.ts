...
> = Effect.gen(function* () {
  ...
  return {
    provider: CLAUDE_CODE_PROVIDER,
    status: parsed.status,
    available: parsed.status === "ready" && parsed.authStatus === "authenticated",
    authStatus: parsed.authStatus,
    checkedAt,
    ...(parsed.message ? { message: parsed.message } : {}),
  } satisfies ServerProviderStatus;
...
export const ProviderHealthLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const claudeCodeStatusFiber = yield* checkClaudeCodeProviderStatus.pipe(Effect.forkScoped);

    return {
      getStatuses: Effect.all([Fiber.join(codexStatusFiber), Fiber.join(claudeCodeStatusFiber)]),
